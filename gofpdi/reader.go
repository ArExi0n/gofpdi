...
func (this *PDFreader) readToken(r *bufio.Reader) (string, error) {
	...
	switch b {
	case '[', ']', '(', ')':
		// this is either an array or literal string delimeter, returns it
		return string(b), nil

	case '<', '>':
		// this could either be a hex string or a dictionary delimeter
		// determine the appropiate case and return the token.
		nb, err := r.ReadByte()
		if err != nil {
			return "", errors.Wrap(err, "failed to read byte")
		}
	...
	default:
		// FIXME: this may not be performent to create new strings to create new strings for each type
		// it is probably better to create a buffer and then convert to a string in the end
		str := string(b)
...
func (this *PDFreader) readValue(r *bufio.Reader, t string) (*PdfValue, error) {
	...
	case "<<":
		...
		//
		// resource into this function until we reach the end of the dictornary
		for {
			...
			if t2 != "" {
				if is_numeric(t2) {
					...
					if t3 != "" {
						...
						// Push the extra token back into the stack and return the value
...
func (this *PDFreader) resolveCompressedObjects(objSpec *PdfValue) (*PdfValue, error) {
	var err error
